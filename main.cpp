...
char man[] = "Models/man_obj.txt";
char planet[] = "Models/planet_obj.txt";
char planet_t[] = "Models/planet_textured_obj.txt";
char USS_M[] = "Models/USS.txt";
char invader[] = "Models/invader.txt";
char USS_T[] = "Textures/USS.bmp";
char T_T[] = "Textures/Tardis.bmp";
char T_M[] = "Models/Time.txt";
...
init()
{
    ...
    glGenVertexArraysAPPLE( 1, &USS_VAO );
    glBindVertexArrayAPPLE( USS_VAO );
    ReadModel2(&USS_Model[0],USS_M);
    UploadModel2(USS_Model[0],USS_T);
    USS_Model[0].crm = USS_Model[0].crm * Translate(dist_from_sun[EAR]+30.0,2.0,0.0) * Scale(.75, .75, .75);
...
display( void )
{
    ...
    //Bind Buffers
    glBindBuffer( GL_ARRAY_BUFFER, USS_Model[0].buffer );
    
    //Bind Texture Buffer
    glBindTexture( GL_TEXTURE_2D, USS_Model[0].uvs_buffer  );
    ...
    //Set the starting index for each array
    glVertexAttribPointer( vPosition, 4, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(USS_Model[0].ogl_v_offset));
    glVertexAttribPointer( vNormal, 3, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(USS_Model[0].ogl_n_offset));
    glVertexAttribPointer( vTexCoord, 2, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(USS_Model[0].ogl_uv_offset));
    
    //Pass model matrix
    glUniformMatrix4fv( Model_I, 1, GL_TRUE, USS_Model[0].crm);
    
    //Draw model
    glDrawArrays( GL_TRIANGLES, 0, USS_Model[0].num_vertices);
...
void idle( void )
{
    ...
    if(start_USS)
    {
        USS_Model[0].crm =  RotateY(-0.2) * USS_Model[0].crm;
    }
    else{
        start_USS = false;
    }
    ...
    if(do_rotation == false){
        switch( key )
        {
            ...
            case ';':
                ...
                if(color_picker >= 8)
                    color_picker = 0;
                light_ambient = colors[color_picker];
                color_picker++;
                break;
            case 'L':
                color_strobe = false;
            ...
            case '[':
                if(light_picker >= lights)
                    light_picker = 0;
                light_position = light_posXZ[light_picker];
                light_picker++;
                break;
            case 'R':
                if(start_orbit)
...
{
    planet_texture[0] = "Textures/sun.bmp";
    planet_texture[1] = "Textures/mer.bmp";
    planet_texture[2] = "Textures/ven.bmp";
    planet_texture[3] = "Textures/ear.bmp";
    ...
    planet_texture[9] = "Textures/stars.bmp";
    
    glutInit( &argc, argv );
    glutInitDisplayMode( GLUT_RGBA | GLUT_DOUBLE | GLUT_DEPTH );
    glutInitWindowSize( 725, 725 );
